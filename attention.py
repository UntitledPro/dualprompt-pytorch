...
class PrefixAttention(nn.Module):
    ...
    def forward(self, x, prompt):
        """
        Param:
            x:      [Batch, Length, Dimention]
            prompt: fetched from [num_layers, batch_size, dual/2, (top_k * length), num_head, head_dim]
                    [batch_size, dual/2, (top_k * length), num_head, head_dim]
        """
        B, N, C = x.shape
        # for q/k/v: [B, N, 3, num_heads, head_dim]
        ...
        if prompt is not None:
            # prefix key, value
            # [2, B, num_heads, prompt_length, head_dim]
            ...
            assert (key_prefix.shape[0], key_prefix.shape[1], key_prefix.shape[3]
                    ) == expected_shape, f'key_prefix.shape: {key_prefix.shape} not match k.shape: {k.shape}'
            assert (value_prefix.shape[0], value_prefix.shape[1], value_prefix.shape[3]
