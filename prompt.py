...
class EPrompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_key_init='uniform',
        ...
        """Exper prompt module for DualPrompt
        Param:
            length:                         prompt length for each task
            embed_dim:                      embedding dimension for each token in prompt
            embedding_key:                  how to calculate embedding key
            prompt_init:                    initialization method for prompt
            prompt_pool:                    whether to use prompt pool
            prompt_key:                     if using learnable prompt keys used for choosing prompt within pool
            prompt_key_init:                initialization method for prompt key
            pool_size:                      number of prompts
            top_k
            batchwise_prompt:               the whole batch share one prompt
            num_layers
            use_prefix_tune_for_e_prompt:   use prefix tuning or prompt tuning
            num_heads:                      number of heads for transformer
            same_key_value:                 whether to use same key and value for each prompt
        """
        super().__init__()
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        """Select expert prompt for image embedding batches
        Param:
            prompt_mask: force current embedding to use certain prompt
                        only used during training
            cls_features: use cls features/token as embedding key
        """
        out = dict()
        if self.prompt_pool:
            ...
            '[B, top_k], [B, top_k]'
            'idx: index for prompts'
            (similarity_top_k, idx) = torch.topk(
                similarity, k=self.top_k, dim=1)  # B, top_k
            out['similarity'] = similarity

            if self.batchwise_prompt:
                prompt_id, id_counts = \
                    torch.unique(idx, return_counts=True, sorted=True)
                ...
                # find the top-k prompt id for batch
                _, major_idx = torch.topk(id_counts, k=self.top_k)  # top_k
                major_prompt_id = prompt_id[major_idx]  # top_k
                # expand to batch
                idx = major_prompt_id.expand(
                    x_embed.shape[0], -1).contiguous()  # B, top_k
            ...
            out['prompt_idx'] = idx
            if self.use_prefix_tune_for_e_prompt:
                # [num_layers, (key,value), B, top_k, length, num_heads, heads_embed_dim]
                batched_prompt_raw = self.prompt[:, :, idx]
                num_layers, dual, batch_size, top_k, length, num_heads, heads_embed_dim \
                    = batched_prompt_raw.shape
                batched_prompt = batched_prompt_raw.reshape(
                    num_layers, batch_size, dual, top_k * length, num_heads, heads_embed_dim
                )
            else:
                # use prompt tuning for e_prompt
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                batched_prompt = self.prompt.unsqueeze(
                    0).expand(-1, x_embed.shape[0], -1, -1, -1)
            else:
                ...
                if self.prompt_init == 'zero':
                    ...
                    nn.init.uniform_(self.prompt, -1, 1)
                batched_prompt = self.prompt.unsqueeze(
                    0).expand(-1, x_embed.shape[0], -1, -1)
